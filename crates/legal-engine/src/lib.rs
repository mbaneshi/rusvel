...
impl LegalEngine {
    pub fn new(
        ...
        Self { storage, events, agent, jobs, contracts, compliance, ip }
    }

    pub fn contracts(&self) -> &ContractManager { &self.contracts }
    pub fn compliance(&self) -> &ComplianceManager { &self.compliance }
    pub fn ip(&self) -> &IpManager { &self.ip }
...
mod tests {
    ...
    async fn contract_create_and_list() {
        ...
        let contract = engine.contracts().create_contract(
            sid,
            "NDA Agreement".into(),
            "Acme Corp".into(),
            "standard_nda".into(),
