...
impl SupportEngine {
    pub fn new(
        ...
        Self { storage, events, agent, jobs, tickets, knowledge, nps }
    }

    pub fn tickets(&self) -> &TicketManager { &self.tickets }
    pub fn knowledge(&self) -> &KnowledgeManager { &self.knowledge }
    pub fn nps(&self) -> &NpsManager { &self.nps }
...
mod tests {
    ...
    async fn ticket_create() {
        ...
        let ticket = engine.tickets().create_ticket(
            sid,
            "Login broken".into(),
