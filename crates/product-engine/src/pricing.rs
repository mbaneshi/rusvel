...
pub struct PricingManager { storage: Arc<dyn StoragePort> }

impl PricingManager {
    pub fn new(storage: Arc<dyn StoragePort>) -> Self { Self { storage } }

    pub async fn create_tier(&self, session_id: SessionId, name: String, price_monthly: f64, price_yearly: Option<f64>, features: Vec<String>) -> Result<PricingTierId> {
        let tier = PricingTier { id: PricingTierId::new(), session_id, name, price_monthly, price_yearly, features, is_active: true, metadata: serde_json::json!({}) };
        let id = tier.id;
        self.storage.objects().put(KIND, &id.to_string(), serde_json::to_value(&tier)?).await?;
        Ok(id)
    }
