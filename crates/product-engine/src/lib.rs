...
pub use roadmap::{RoadmapManager, Feature, FeatureId, FeatureStatus, Priority};
pub use pricing::{PricingManager, PricingTier, PricingTierId};
pub use feedback::{FeedbackManager, Feedback, FeedbackId, FeedbackKind};
...
impl ProductEngine {
    pub fn new(
        ...
        Self { storage, events, agent, jobs, roadmap, pricing, feedback }
    }

    pub fn roadmap(&self) -> &RoadmapManager { &self.roadmap }
    pub fn pricing(&self) -> &PricingManager { &self.pricing }
    pub fn feedback(&self) -> &FeedbackManager { &self.feedback }

    pub async fn emit_event(&self, kind: &str, payload: serde_json::Value) -> Result<EventId> {
        let event = Event {
            id: EventId::new(), session_id: None, run_id: None,
            source: EngineKind::Product, kind: kind.into(), payload,
...
impl rusvel_core::engine::Engine for ProductEngine {
    ...
    async fn health(&self) -> Result<HealthStatus> {
        Ok(HealthStatus { healthy: true, message: None, metadata: serde_json::json!({}) })
    }
}
...
mod tests {
    ...
    struct StubEvents; struct StubSessions; struct StubJobStore; struct StubMetrics;
    ...
    #[async_trait] impl ObjectStore for StubObjects {
        async fn put(&self, kind: &str, id: &str, object: serde_json::Value) -> Result<()> {
            ...
            data.push((kind.into(), id.into(), object)); Ok(())
        }
        ...
        async fn delete(&self, kind: &str, id: &str) -> Result<()> {
            let mut data = self.data.lock().unwrap();
            data.retain(|(k, i, _)| !(k == kind && i == id)); Ok(())
        }
    ...
    fn make_engine() -> ProductEngine {
        ProductEngine::new(Arc::new(StubStore::new()), Arc::new(StubEventPort), Arc::new(StubAgentPort), Arc::new(StubJobPort))
    }
    ...
    async fn roadmap_add_feature() {
        ...
        engine.roadmap().add_feature(sid, "Auth system".into(), "OAuth2 + JWT".into(), Priority::High, Some("v1.0".into())).await.unwrap();
    ...
    async fn pricing_create_tier() {
        let engine = make_engine();
        let sid = SessionId::new();
        engine.pricing().create_tier(sid, "Pro".into(), 29.0, Some(290.0), vec!["unlimited".into()]).await.unwrap();
