...
pub struct FunnelManager { storage: Arc<dyn StoragePort> }

impl FunnelManager {
    pub fn new(storage: Arc<dyn StoragePort>) -> Self { Self { storage } }

    pub async fn add_stage(&self, session_id: SessionId, name: String, order: u32) -> Result<FunnelStageId> {
        let stage = FunnelStage { id: FunnelStageId::new(), session_id, name, order, visitors: 0, conversions: 0, metadata: serde_json::json!({}) };
        let id = stage.id;
        self.storage.objects().put(KIND, &id.to_string(), serde_json::to_value(&stage)?).await?;
        Ok(id)
    }
    ...
    pub async fn list_stages(&self, session_id: SessionId) -> Result<Vec<FunnelStage>> {
        ...
        let mut stages: Vec<FunnelStage> = vals.into_iter().filter_map(|v| serde_json::from_value(v).ok()).collect();
