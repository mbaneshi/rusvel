...
pub use cohort::{CohortManager, Cohort, CohortId};
pub use kpi::{KpiManager, KpiEntry, KpiId};
...
impl GrowthEngine {
    pub fn new(
        ...
        Self { storage, events, agent, jobs, funnel, cohort, kpi }
    }

    pub fn funnel(&self) -> &FunnelManager { &self.funnel }
    pub fn cohort(&self) -> &CohortManager { &self.cohort }
    pub fn kpi(&self) -> &KpiManager { &self.kpi }

    pub async fn emit_event(&self, kind: &str, payload: serde_json::Value) -> Result<EventId> {
        let event = Event {
            id: EventId::new(), session_id: None, run_id: None,
            source: EngineKind::Growth, kind: kind.into(), payload,
...
mod tests {
    ...
    fn make_engine() -> GrowthEngine {
        GrowthEngine::new(Arc::new(StubStore::new()), Arc::new(StubEventPort), Arc::new(StubAgentPort), Arc::new(StubJobPort))
    }
    ...
    async fn funnel_add_stage() {
        ...
        engine.funnel().add_stage(sid, "Awareness".into(), 1).await.unwrap();
    ...
    async fn kpi_record() {
        let engine = make_engine();
        let sid = SessionId::new();
        engine.kpi().record_kpi(sid, "MRR".into(), 5000.0, "USD".into()).await.unwrap();
