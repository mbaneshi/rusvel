...
impl InfraEngine {
    pub fn new(
        ...
        Self { storage, events, agent, jobs, deploy, monitor, incidents }
    }

    pub fn deploy(&self) -> &DeployManager { &self.deploy }
    pub fn monitor(&self) -> &MonitorManager { &self.monitor }
    pub fn incidents(&self) -> &IncidentManager { &self.incidents }
...
mod tests {
    ...
    async fn deploy_record() {
        ...
        let deployment = engine.deploy().record_deployment(
            sid,
            "rusvel-api".into(),
            "v0.2.0".into(),
            "production".into(),
