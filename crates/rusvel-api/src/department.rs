...
// ── Code Department Wrappers ─────────────────────────────────

pub async fn code_chat(
    State(state): State<Arc<AppState>>,
    Json(body): Json<ChatRequest>,
) -> Result<Sse<impl Stream<Item = Result<Event, Infallible>>>, (StatusCode, String)> {
    department_chat_handler("code", state, body).await
}

pub async fn code_config_get(
    State(state): State<Arc<AppState>>,
) -> Result<Json<DepartmentConfig>, (StatusCode, String)> {
    get_dept_config("code", &state).await.map(Json)
}

pub async fn code_config_update(
    State(state): State<Arc<AppState>>,
    Json(config): Json<DepartmentConfig>,
) -> Result<Json<DepartmentConfig>, (StatusCode, String)> {
    update_dept_config("code", &state, config).await.map(Json)
}

pub async fn code_conversations(
    State(state): State<Arc<AppState>>,
) -> Result<Json<Vec<ConversationSummary>>, (StatusCode, String)> {
    list_dept_conversations("code", &state).await.map(Json)
}
...
pub async fn code_history(
    ...
    get_dept_history("code", &state, &id).await.map(Json)
}

pub async fn code_events(
    State(state): State<Arc<AppState>>,
) -> Result<Json<Vec<rusvel_core::domain::Event>>, (StatusCode, String)> {
    get_dept_events(EngineKind::Code, &state).await.map(Json)
}
