//! Distribution Engine — marketplace listings, SEO tracking, and affiliate partnerships.
...
pub use affiliate::{AffiliateManager, Partner, PartnerId};
pub use marketplace::{Listing, ListingId, ListingStatus, MarketplaceManager};
pub use seo::{Keyword, KeywordId, SeoManager};

pub mod events {
    pub const LISTING_PUBLISHED: &str = "distro.listing.published";
    pub const SEO_RANKED: &str = "distro.seo.ranked";
    pub const AFFILIATE_JOINED: &str = "distro.affiliate.joined";
    pub const PARTNERSHIP_CREATED: &str = "distro.partnership.created";
}

pub struct DistroEngine {
    #[allow(dead_code)]
...
impl DistroEngine {
    ...
    pub async fn emit_event(&self, kind: &str, payload: serde_json::Value) -> Result<EventId> {
        let event = Event {
            ...
            session_id: None,
            run_id: None,
            source: EngineKind::Distribution,
            kind: kind.into(),
            payload,
...
impl rusvel_core::engine::Engine for DistroEngine {
    ...
    fn capabilities(&self) -> Vec<Capability> {
        vec![
            Capability::Custom("marketplace".into()),
            Capability::Custom("seo".into()),
            Capability::Custom("affiliate".into()),
        ]
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn marketplace_add_listing() {
        ...
        let all = engine.marketplace().list_listings(sid).await.unwrap();
        assert_eq!(all.len(), 1);
    }
