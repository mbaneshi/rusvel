//! Legal Department — DepartmentApp implementation.
//!
//! Wraps `legal-engine` (contracts, compliance, IP) with the
...
use legal_engine::LegalEngine;
use rusvel_core::Engine;
use rusvel_core::department::*;
use rusvel_core::error::Result;

pub struct LegalDepartment {
    engine: OnceLock<Arc<LegalEngine>>,
}
...
impl LegalDepartment {
    ...
    pub fn engine(&self) -> Option<&Arc<LegalEngine>> {
        self.engine.get()
    }
}

impl Default for LegalDepartment {
    fn default() -> Self {
        Self::new()
    }
}
