<script lang="ts">
	...
	let { dept, config = $bindable(null) }: { dept: string; config: DepartmentConfig | null } = $props();
	...
	async function addDir() {
		...
		if (dir && !config.add_dirs.includes(dir)) {
			...
			} catch (e) { toast.error(`Failed to add directory: ${e instanceof Error ? e.message : e}`); }
		}
	}
	...
	async function removeDir(dir: string) {
		...
		} catch (e) { toast.error(`Failed to remove directory: ${e instanceof Error ? e.message : e}`); }
	}
</script>
	...
	{#if config}
		{#each config.add_dirs as dir}
			<div class="flex items-center justify-between rounded-lg bg-secondary px-3 py-2">
				...
				<button onclick={() => removeDir(dir)} class="text-muted-foreground hover:text-destructive text-xs">x</button>
			</div>
		{/each}
		<button onclick={addDir} class="w-full rounded-lg border border-dashed border-border py-2 text-xs text-muted-foreground hover:text-foreground">
			+ Add directory
		</button>
	{/if}
</div>
