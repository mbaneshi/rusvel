<script lang="ts">
	...
	let { dept, title = 'Department', icon = 'D', suggestedPrompts = [] }: { dept: string; title?: string; icon?: string; suggestedPrompts?: string[] } = $props();
	...
	async function send() {
		...
		if (text.startsWith('/help')) {
			...
			try {
				await streamHelp(question, dept,
					(delta) => {
			...
			} catch (e) { error = e instanceof Error ? e.message : 'Help failed'; if (messages[messages.length - 1]?.content === '') messages = messages.slice(0, -1); sending = false; }
			return;
		}
...
<div class="flex h-full flex-col">
	...
	<div class="flex items-center justify-between border-b border-border bg-card px-3 py-2">
		<div class="flex items-center gap-2">
			<div class="flex h-6 w-6 items-center justify-center rounded-md bg-chart-2/30 text-[10px] font-bold text-chart-2">{icon}</div>
			<span class="text-sm font-medium text-foreground">{title}</span>
		</div>
		<div class="flex items-center gap-1">
			<button onclick={() => showConfig = !showConfig} title="Settings" class="rounded-md p-1 text-muted-foreground hover:bg-accent hover:text-accent-foreground {showConfig ? 'text-foreground bg-accent' : ''}">
			...
			<button onclick={() => showHistory = !showHistory} title="History" class="rounded-md p-1 text-muted-foreground hover:bg-accent hover:text-accent-foreground">
			...
			<button onclick={newChat} title="New chat" class="rounded-md p-1 text-muted-foreground hover:bg-accent hover:text-accent-foreground">
	...
	{#if showConfig && config}
		<div class="border-b border-border bg-card px-3 py-2 space-y-2">
			<div class="flex items-center gap-2">
				...
				<select value={config.model} onchange={setModel} class="rounded-md border border-border bg-secondary px-2 py-0.5 text-xs text-foreground focus:outline-none">
				...
				<div class="flex rounded-md border border-border bg-secondary">
					{#each ['low','medium','high','max'] as level}
						<button onclick={() => setEffort(level)} class="px-1.5 py-0.5 text-[10px] {config.effort === level ? 'bg-primary text-primary-foreground' : 'text-muted-foreground'}">{level}</button>
					{/each}
				</div>
			</div>
			<div class="flex flex-wrap gap-1">
				{#each tools as tool}
					...
					<button onclick={() => toggleTool(tool.name)} class="rounded px-1.5 py-0.5 text-[10px] border {enabled ? 'border-primary/50 text-primary bg-primary/10' : 'border-border text-muted-foreground line-through opacity-50'}">
						{tool.name}
					</button>
				{/each}
			</div>
	...
	{#if showHistory}
		<div class="max-h-40 overflow-y-auto border-b border-border bg-card">
			{#each conversations as conv}
				<button onclick={() => loadConversation(conv.id)} class="w-full px-3 py-1.5 text-left text-xs hover:bg-accent {conversationId === conv.id ? 'bg-accent' : 'text-muted-foreground'}">
					<p class="truncate">{conv.title}</p>
				</button>
			{/each}
			{#if conversations.length === 0}<p class="p-3 text-center text-xs text-muted-foreground">No conversations</p>{/if}
		</div>
	{/if}
		...
		{#if messages.length === 0}
			<div class="flex h-full items-center justify-center p-4">
				<div class="text-center">
					<div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-chart-2/20">
						<span class="text-xl font-bold text-chart-2">{icon}</span>
					</div>
					<p class="text-sm font-medium text-foreground">{title}</p>
					<p class="mt-1 text-xs text-muted-foreground">Ready to work. Ask anything or try a suggestion below.</p>
					{#if suggestedPrompts.length > 0}
						<div class="mt-4 flex flex-wrap justify-center gap-1.5">
							{#each suggestedPrompts as prompt}
								<button
									onclick={() => { inputText = prompt; send(); }}
						...
						<div class="max-w-[85%] rounded-xl px-3 py-2 text-xs leading-relaxed {isUser ? 'bg-primary text-primary-foreground rounded-br-sm' : 'bg-secondary text-foreground rounded-bl-sm'}">
							...
							{:else if msg.streaming && !msg.content}
								<div class="flex items-center gap-1">
									...
									<div class="h-1.5 w-1.5 animate-bounce rounded-full bg-chart-2" style="animation-delay:150ms"></div>
									<div class="h-1.5 w-1.5 animate-bounce rounded-full bg-chart-2" style="animation-delay:300ms"></div>
								</div>
							{:else}
	...
	{#if error}
		<div class="mx-2 mb-1 rounded-md bg-destructive/10 px-2 py-1 text-[10px] text-destructive">{error} <button onclick={() => error = ''} class="underline">dismiss</button></div>
	{/if}

	<!-- Input -->
	<div class="border-t border-border p-2">
		<div class="flex items-end gap-2">
			<textarea bind:this={textareaEl} bind:value={inputText} onkeydown={keydown} oninput={autoResize} placeholder="Ask {title}..." rows="1" disabled={sending} class="flex-1 resize-none rounded-lg border border-input bg-background px-3 py-2 text-xs text-foreground placeholder-muted-foreground focus:outline-2 focus:outline-offset-2 focus:outline-ring disabled:opacity-50"></textarea>
			<button onclick={send} disabled={sending || !inputText.trim()} class="flex h-7 w-7 items-center justify-center rounded-lg bg-primary hover:bg-primary/90 disabled:opacity-30">
				{#if sending}<div class="h-3 w-3 animate-spin rounded-full border-2 border-primary-foreground/30 border-t-primary-foreground"></div>
				{:else}<svg class="h-3 w-3 text-primary-foreground" viewBox="0 0 20 20" fill="currentColor"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" /></svg>{/if}
			</button>
		</div>
	</div>
</div>
